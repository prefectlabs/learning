...
async def project_browser(ctx: Context) -> PrefabApp:
    """Render a project browser with placeholder buttons."""
    active_filter = "all"
    projects = filter_projects(active_filter)

    with Column(gap=6, cssClass="p-6") as view:
        Heading("Project Browser")
        Text("Replace this placeholder with buttons that persist filters and selections in MCP state.")
        for project in projects:
            with Column(gap=2, cssClass="rounded-lg border p-4"):
                Heading(project["name"], level=3)
                Text(f"Current status: {project_status_label(project['status'])}")
                Button("View details")
...
async def selected_project_details(ctx: Context) -> PrefabApp:
    ...
    project = find_project("proj-api")
    with Column(gap=6, cssClass="p-6") as view:
        Heading("Selected Project")
        Text("Replace this placeholder by reading the selected project from the MCP context.")
        if project:
            Heading(project["name"], level=3)
            Text(project["focus"])
