...
ACTIVE_FILTER_KEY = "active_board_filter"
...
@mcp.tool(app=True)
async def project_browser(ctx: Context) -> PrefabApp:
    """Render a filterable project browser backed by MCP session state."""
    active_filter = await ctx.get_state(ACTIVE_FILTER_KEY) or "all"
    selected_project_id = await ctx.get_state(SELECTED_PROJECT_KEY)
    projects = filter_projects(active_filter)

    with Column(gap=6, cssClass="p-6") as view:
        Heading("Project Browser")
        Text("Persist the active filter and selected project between tool calls.")

        with Row(gap=2):
            for board_filter in ["all", "on-track", "at-risk", "blocked"]:
                Button(
                    project_status_label(board_filter),
                    variant="secondary" if board_filter == active_filter else "outline",
                    onClick=CallTool(
                        "remember_board_filter",
                        arguments={"board_filter": board_filter},
                        onSuccess=CallTool("project_browser"),
                    ),
                )

        if selected_project_id:
            Badge(f"Selected: {selected_project_id}", variant="info")
...
@mcp.tool
async def remember_board_filter(ctx: Context, board_filter: str) -> dict:
    """Persist the active project filter."""
    await ctx.set_state(ACTIVE_FILTER_KEY, board_filter)
    return {"board_filter": board_filter}


@mcp.tool
async def remember_selected_project(ctx: Context, project_id: str) -> dict:
    """Persist the selected project identifier."""
    await ctx.set_state(SELECTED_PROJECT_KEY, project_id)
    return {"project_id": project_id}


@mcp.tool(app=True)
async def selected_project_details(ctx: Context) -> PrefabApp:
    """Render the selected project detail card."""
    project_id = await ctx.get_state(SELECTED_PROJECT_KEY)
    active_filter = await ctx.get_state(ACTIVE_FILTER_KEY) or "all"
    ...
    with Column(gap=6, cssClass="p-6") as view:
        ...
        if project is None:
            Text("Choose a project from Project Browser first.")
        else:
            with Row(gap=2, align="center"):
                Heading(project["name"], level=2)
                Badge(project_status_label(project["status"]), variant=project_status_variant(project["status"]))

            Text(f"Owner: {project['owner']}")
            Text(f"Sprint: {project['sprint']}")
            Text(f"Completion: {project['completion']}%")
            Text(f"Open bugs: {project['open_bugs']}")
            Text(f"Focus: {project['focus']}")
            Badge(f"Active filter: {project_status_label(active_filter)}", variant="outline")
